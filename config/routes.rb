...
TestWeixin::Application.routes.draw do
  ...
  scope :path => "/weixin", :via => :post do 
    ...
    root :to => 'weixin#subscribe_message', :constraints => Weixin::Router.new(:type => "event", :event => "subscribe")
    root :to => 'weixin#click_menu_message', :constraints => Weixin::Router.new(:type => "event", :event => "CLICK")
  end
end